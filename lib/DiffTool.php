...
$DiffToolDir = dirname(__FILE__);

require_once $DiffToolDir . '/JsonFileReader.php';
require_once $DiffToolDir . '/XmlFileReader.php';
...
class DiffTool {
    ...
    */
    ...
    /**
     * 根据解析后的字符串，构建同 key 数组和不同 key 数组。
     * @param array 同 key 数组
     * @param array 不同 key 数组
     * @param array 解析后的字符串数组
     * @return int, 执行正确返回 0，错误返回错误码
    */
    private function generateKeys(&$arrSameKeys, &$arrDiffKeys, $arrParsedLineList) {
    ...
    /**
     * 根据不同 key 数组，生成对 key 的 diff 对比 。
     * @param array 不同 key 数组
     * @param array 解析后的字符串数组
     * @param FileReader 输出对象
     * @return int, 执行正确返回 0，错误返回错误码
    */
    private function keyDiff($arrDiffKeys, $arrParsedLineList, &$reader) {
    ...
    /**
     * 根据相同 key 数组，生成对 value 的 diff 对比 。
     * @param array 相同 key 数组
     * @param array 解析后的字符串数组
     * @param FileReader 输出对象
     * @return int, 执行正确返回 0，错误返回错误码
    */
    private function valueDiff($arrSameKeys, $arrParsedLineList, &$reader) {
    ...
    */
    public function diff(&$reader) {
        while (1) {
            ...
            $arrSameKeys = array();
            $arrDiffKeys = array();
            $this->generateKeys($arrSameKeys, $arrDiffKeys, $arrParsedLineList);
            
            // 对不同 key 做 diff 判断
            $this->keyDiff($arrDiffKeys, $arrParsedLineList, $reader);
            // 对不同 value 做 diff 判断
            $this->valueDiff($arrSameKeys, $arrParsedLineList, $reader);
